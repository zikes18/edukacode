...
public class PersonagemController {
    ...
    @PutMapping
    public String atualizarGenero(@RequestBody DadosAtualizacaoPersonagem dados) {
        var lead = repository.getReferenceById(dados.id());
        lead.atualizarInformacoes(dados);
        return "Informações do personagem atualizado com sucesso!";
    }

    @DeleteMapping("/{id}")
    public String excluirPersonagem(@PathVariable Long id) {
        repository.deleteById(id);
        return "Personagem "+id+" deletado com sucesso.";
    }
